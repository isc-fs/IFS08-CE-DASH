/* Porting note:
 * Base template copied from ST's N25Q256A STM32L476 example.
 * This working copy is being adapted toward MT25QL128A on STM32H743
 * with CUSTOM7 pinout: PB2 CLK, PB10 NCS, PF8 IO0, PF9 IO1, PF7 IO2, PF6 IO3.
 * The flash geometry and pin mapping below already reflect the target design,
 * but the low-level peripheral layer is still the legacy STM32L4 QSPI library.
 */
/* Includes ------------------------------------------------------------------*/
...
void QUADSPI_Config(void)
{
  ...
  /* 16 MiB external flash => 24 address bits => FSIZE = 23 (0x17). */
  QUADSPI_InitStructure.QUADSPI_FSize = 0x17;
...
void GPIO_Clock_Config(void)
{
  ...
  /* Enable GPIO AHB2 clock */
   RCC_AHB2PeriphClockCmd(RCC_AHB2Periph_GPIOB|RCC_AHB2Periph_GPIOF,ENABLE);
 ...
 * PF6  -> IO3
 */
   GPIO_PinAFConfig(GPIOB,GPIO_PinSource2,GPIO_AF_QSPI);
